...
public class Node {
    ...
    private final boolean debug = false; // flag for logDebug messages
    ...
    public void onReceive(Integer from, byte[] msg) {
        ...
        if (packet == null) {
            logError("Unable to unpack message: "
                    + Utility.byteArrayToString(msg) + " Received from " + from);
            return;
        }
    ...
    private boolean matchPingCommand(String command) {
        ...
        try {
            ...
            Packet packet = new Packet(destAddr, this.addr, Packet.MAX_TTL,
                    Protocol.PING_PKT, 0, Utility.stringToByteArray(message));
    ...
    private void receivePacket(int from, Packet packet) {
        ...
        case Protocol.TRANSPORT_PKT:
            this.receiveTransportPacket(packet, from);
            break;
    ...
    private void receiveTransportPacket(Packet packet, int src) {
        //logOutput("Received TRANSPORT packet from " + src + ".");
        tcpMan.handlePacket(Transport.unpack(packet.getPayload()), src);
    }
    ...
    private void receivePing(Packet packet) {
        ...
        try {
            Packet reply = new Packet(packet.getSrc(), this.addr,
                    Packet.MAX_TTL, Protocol.PING_REPLY_PKT, 0,
                    packet.getPayload());
            this.send(packet.getSrc(), reply);
        } catch (IllegalArgumentException e) {
            logError("Exception while trying to send a Ping Reply. Exception: "
                    + e);
        }
    }
    ...
    private void receivePingReply(Packet packet) {
        ...
        while (iter.hasNext()) {
            ...
            if ((pingRequest.getDestAddr() == packet.getSrc())
                    && (Utility.byteArrayToString(pingRequest.getMsg())
                            .equals(payload))) {

                logOutput("Got Ping Reply from " + packet.getSrc() + ": "
                        + payload);
                try {
                    iter.remove();
                } catch (Exception e) {
        ...
        logError("Unexpected Ping Reply from " + packet.getSrc() + ": "
                + payload);
    }
     ...
     */
    public void sendSegment(int srcAddr, int destAddr, int protocol,
            byte[] payload) {
        Packet packet = new Packet(destAddr, srcAddr, Packet.MAX_TTL, protocol,
                0, payload);
        this.send(destAddr, packet);
    }
    ...
    private boolean matchTransferCommand(String command) {
        ...
        try {
            ...
            long interval = args.length >= 6 ? Integer.parseInt(args[5])
                    : TransferClient.DEFAULT_CLIENT_INTERVAL;
            int sz = args.length == 7 ? Integer.parseInt(args[6])
                    : TransferClient.DEFAULT_BUFFER_SZ;

            TCPSock sock = this.tcpMan.socket();
            sock.bind(localPort);
            sock.connect(destAddr, port);
            TransferClient client = new TransferClient(manager, this, sock,
                    amount, interval, sz);
            client.start();
    ...
    private boolean matchServerCommand(String command) {
        ...
        try {
            ...
            long servint = args.length >= 4 ? Integer.parseInt(args[3])
                    : TransferServer.DEFAULT_SERVER_INTERVAL;
            long workint = args.length >= 5 ? Integer.parseInt(args[4])
                    : TransferServer.DEFAULT_WORKER_INTERVAL;
            int sz = args.length == 6 ? Integer.parseInt(args[5])
                    : TransferServer.DEFAULT_BUFFER_SZ;
            TCPSock sock = this.tcpMan.socket();
            sock.bind(port);
            sock.listen(backlog);

            TransferServer server = new TransferServer(manager, this, sock,
                    servint, workint, sz);
            server.start();
